...
export class AuthService {
  ...
  login(username: string, password: string) {
    ...
    return this.http.post(`/api/auth/login`, body).pipe(tap(() => this.refreshCurrentUser(true)));
  }
  ...
  logout() {
    console.log('logout');
    const body = {};
    return this.http.post(`/api/auth/logout`, body).pipe(tap(() => this.refreshCurrentUser(true)));
  }
